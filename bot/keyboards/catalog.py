...
def categories_kb(categories: list[Category]) -> InlineKeyboardMarkup:
    buttons = [[Btn(text=f"{cat.emoji} {cat.name}", callback_data=f"cat:{cat.id}")] for cat in categories]
    buttons.append([Btn(text="\u2b05\ufe0f \u041d\u0430\u0437\u0430\u0434", callback_data="main_menu")])
...
def product_card_kb(
    ...
    if idx > 0:
        nav_buttons.append(Btn(text="\u2b05\ufe0f", callback_data=f"prod:{category_id}:{idx - 1}"))
